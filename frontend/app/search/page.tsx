...
import type { Subject, Grade, PublicTeacher } from '@/src/lib/types';
...
function SearchPageInner() {
  ...
  const [catalog, setCatalog] = useState<{ subjects: Subject[]; grades: Grade[] }>({
    subjects: [],
    grades: [],
  });
  ...
  useEffect(() => {
    Promise.all([
      ...
      setCatalog({ subjects: subjects.data ?? [], grades: grades.data ?? [] });
    });
  }, []);
  ...
  if (loading && teacherResults.length === 0) {
    return (
          ...
          <>
            <div className="mb-6 flex items-center justify-between">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {teacherResults.length} {t('teachersFound')}
              </p>
              <Pagination
                page={debounced.page}
