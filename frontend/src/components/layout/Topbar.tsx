...
export function Topbar({ onOpenSidebar }: { onOpenSidebar: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/80 px-4 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/80 sm:px-6">
      ...
      <div className="flex items-center gap-1">
        ...
        <div ref={menuRef} className="relative ms-1">
          <button
            ...
            className={`flex items-center gap-2 rounded-full p-1 transition-all duration-150 hover:bg-slate-100 dark:hover:bg-slate-800 ${menuOpen ? 'bg-slate-100 dark:bg-slate-800' : ''}`}
          >
