      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700 text-start text-xs uppercase tracking-wide text-slate-400">
            <th className="pb-2 pe-4 font-medium">{t('paymentId')}</th>
            <th className="pb-2 pe-4 font-medium">{t('payer')}</th>
            {showStudent && <th className="pb-2 pe-4 font-medium">{t('student')}</th>}
            {showParent && <th className="pb-2 pe-4 font-medium">{t('parent')}</th>}
            {showTeacher && <th className="pb-2 pe-4 font-medium">{t('teacher')}</th>}
            <th className="pb-2 pe-4 font-medium">{t('amount')}</th>
            <th className="pb-2 pe-4 font-medium">{t('method')}</th>
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
          {payments.map((p) => (
            <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/40 dark:bg-slate-800/60">
              <td className="py-3 pe-4 font-medium text-slate-800 dark:text-slate-100">{p.paymentNumber}</td>
              <td className="py-3 pe-4 text-slate-600 dark:text-slate-300">{p.payerName}</td>
              {showStudent && <td className="py-3 pe-4">{p.student?.fullName ?? '—'}</td>}
              {showParent && <td className="py-3 pe-4">{p.parent?.fullName ?? '—'}</td>}
              {showTeacher && <td className="py-3 pe-4">{p.teacher?.fullName ?? '—'}</td>}
              <td className="py-3 pe-4 font-medium text-slate-800 dark:text-slate-100">{formatCurrency(p.amount)}</td>
              <td className="py-3 pe-4 text-slate-500">{p.methodLabel}</td>
              <td className="py-3 pe-4">
                <Badge tone={STATUS_TONE[p.status]}>{t(paymentStatusKey(p.status))}</Badge>
              </td>
              <td className="py-3 pe-4 text-slate-500">{formatDate(p.createdAt)}</td>
              <td className="py-3 pe-4">
