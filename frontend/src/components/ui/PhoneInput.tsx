...
export function PhoneInput({ label, value, countryCode, onValueChange, onCountryChange, error, hint, placeholder = '1234567890', disabled }: Props) {
  ...
  return (
    <div className="box-border w-full max-w-full min-w-0" style={{ boxSizing: 'border-box' }}>
      ...
      <div className="flex min-w-0 max-w-full flex-col gap-2 [box-sizing:border-box] min-[360px]:flex-row" style={{ boxSizing: 'border-box' }}>
        <select
          value={countryCode}
          onChange={(e) => onCountryChange(e.target.value)}
          disabled={disabled}
          className="box-border w-full min-w-0 max-w-full flex-shrink-0 rounded-lg border border-slate-300 bg-white px-2 py-2 text-sm text-slate-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 min-[360px]:w-[108px] sm:w-36"
        ...
        <input
          ...
          disabled={disabled}
          inputMode="tel"
          className={`box-border w-full min-w-0 max-w-full flex-1 rounded-lg border bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 dark:bg-slate-900 dark:text-slate-100 dark:placeholder:text-slate-500 min-[360px]:w-auto ${
            error
