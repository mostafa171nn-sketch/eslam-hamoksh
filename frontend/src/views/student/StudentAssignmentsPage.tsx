...
export default function StudentAssignmentsPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (!file && !textAnswer.trim()) {
      setFormError('Upload a file or write an answer to submit.');
      return;
    }
    setSaving(true);
    setFormError('');
  ...
  return (
    <div>
      <PageHeader title="Homework" subtitle="Assignments from your teachers." />
      ...
      {!loading && data && (
        <>
          {data.length === 0 ? (
            <EmptyState icon={FileText} title="No assignments" description="Assignments will show up here once your teachers post them." />
          ) : (
                ...
                <Card key={a.id} bodyClassName="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      {a.teacher.fullName} · Due {formatDateTime(a.deadline)}
                    </p>
                    {a.submission?.grade !== null && a.submission?.grade !== undefined && (
                      <p className="mt-1 text-xs text-slate-600 dark:text-slate-300">
                        Grade: <span className="font-semibold text-slate-900 dark:text-white">{a.submission.grade}/100</span>
                  ...
                  <Button size="sm" variant={a.submission ? 'outline' : 'primary'} onClick={() => openSubmit(a)}>
                    {a.submission ? 'View / resubmit' : 'Submit'}
                  </Button>
                </Card>
              ))}
            </div>
      ...
      <Modal
        ...
        title={active?.title ?? 'Submit homework'}
        footer={
          <>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          {active?.submission && (
            <div className="rounded-lg bg-slate-50 dark:bg-slate-800 p-3 text-xs text-slate-600 dark:text-slate-300">
              <p className="font-medium text-slate-800 dark:text-slate-100">Last submission · {formatDateTime(active.submission.submittedAt)}</p>
              {active.submission.file && (
                <a href={active.submission.file} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-brand-600 hover:underline">
                  <Paperclip className="h-3.5 w-3.5" /> Open submitted file
                </a>
              )}
            </div>
          )}
          <div>
            <p className="mb-1 text-sm font-medium text-slate-700">Upload a file</p>
            <input
              type="file"
              onChange={(e) => setFile(e.target.files?.[0] ?? null)}
              className="block w-full cursor-pointer rounded-lg border border-slate-300 bg-white text-sm text-slate-500 file:mr-3 file:cursor-pointer file:rounded-md file:border-0 file:bg-brand-50 file:px-3 file:py-2 file:text-sm file:font-medium file:text-brand-700 hover:file:bg-brand-100"
            />
          </div>
