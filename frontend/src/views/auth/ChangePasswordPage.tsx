...
export default function ChangePasswordPage() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      setDone(true);
      toast.success(t('changePasswordToast'));
    } catch (err) {
  ...
  if (!token) {
    return (
      <AuthLayout title={t('changePasswordTitle')} subtitle={t('changePasswordSubtitle')}>
  ...
  if (done) {
    return (
      <AuthLayout title={t('passwordChangedDoneTitle')} subtitle={t('passwordChangedDoneSub')}>
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-6 text-center">
          <CheckCircle2 className="mx-auto h-10 w-10 text-emerald-600" />
          <p className="mt-3 text-sm text-emerald-800">{t('passwordChangedDoneSub')}</p>
