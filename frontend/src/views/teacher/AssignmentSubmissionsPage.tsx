...
export default function AssignmentSubmissionsPage() {
  ...
  const saveGrade = async () => {
    ...
    try {
      ...
      toast.success(t('submissionGradedToast'));
      setGrading(null);
      reload();
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        title={assignment ? assignment.title : t('submissionsTitle')}
        subtitle={t('submissionsSub')}
      />

      {error && <Alert message={error} className="mb-4" />}
      {loading && (initialLoading ? <PencilLoader label={t('loadingSubmissions')} /> : <PencilLoader size="sm" label={t('loadingSubmissions')} />)}

      {!loading && data && (
        <>
          {data.length === 0 ? (
            <EmptyState icon={ClipboardList} title={t('noStudentsAssigned')} description={t('assignmentNoStudentsYet')} />
          ) : (
                    ...
                    <div className="flex min-w-0 items-start gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge tone={statusTone(row.status)}>{t(submissionStatusKey(row.status))}</Badge>
                        </div>
                        {sub ? (
                          <>
                            <p className="mt-0.5 text-xs text-slate-500">{t('submitted')} {formatDateTime(sub.submittedAt)}</p>
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-3">
                              {sub.file && (
                                <a href={sub.file} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-medium text-brand-600 hover:text-brand-700">
                                  <Paperclip className="h-3.5 w-3.5" /> {t('downloadFile')}
                                </a>
                              )}
                              {sub.grade !== null && (
                                <Badge tone={sub.grade >= 50 ? 'green' : 'red'}>{t('gradeLabel')}: {sub.grade}/100</Badge>
                              )}
                              {sub.feedback && <span className="text-xs text-slate-500">"{sub.feedback}"</span>}
                            </div>
                          </>
                        ) : (
                          <p className="mt-1 text-xs text-slate-400">{t('notSubmittedYet')}</p>
                        )}
                      </div>
                    </div>
                    <div className="shrink-0">
                      {sub ? (
                        <Button size="sm" variant="outline" onClick={() => openGrading(row)}>
                          {sub.grade !== null ? t('updateGrade') : t('gradeBtn')}
                        </Button>
                      ) : null}
                    </div>
                  </Card>
        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
          ...
          <div className="animate-slide-in relative w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-slate-900 dark:text-white">{t('gradeSubmission')}</h2>
              <button onClick={() => setGrading(null)} className="rounded-md p-1 text-slate-400 hover:bg-slate-100" aria-label={t('close')}>
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="flex justify-end gap-3">
                ...
                <Button onClick={saveGrade} loading={saving} disabled={grading.grade === ''}>
                  <CheckCircle2 className="h-4 w-4" /> {t('saveGrade')}
                </Button>
              </div>
            </div>
          </div>
