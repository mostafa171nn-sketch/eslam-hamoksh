...
export interface CentersViewProps {
  ...
  /** Server-rendered catalog reference data (subjects / grades). */
  initialCatalog?: { subjects: Subject[]; grades: Grade[] } | null;
}

export default function CentersView({ initialResult, initialCatalog }: CentersViewProps) {
  const { t } = useT();
  ...
  const { data, loading, initialLoading, error } = useApi(
    ...
    {
      cacheKey: `centers:search:${debounced.q}:${debounced.city}:${debounced.subject}:${debounced.grade}:${debounced.page}`,
      staleTTL: 30_000,
      cacheTTL: 120_000,
