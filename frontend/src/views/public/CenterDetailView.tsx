...
export interface CenterDetailViewProps {
  initialCenter: PublicCenter;
  initialTeachers: PublicCenterTeacher[];
...
export default function CenterDetailView({ initialCenter, initialTeachers, initialRating }: CenterDetailViewProps) {
  ...
  const [center, setCenter] = useState<PublicCenter | null>(initialCenter);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  // Teachers are loaded separately so the count ALWAYS matches the list below.
  const [teachers, setTeachers] = useState<PublicCenterTeacher[] | null>(initialTeachers);
  const [teachersLoading, setTeachersLoading] = useState(false);
  const [teachersError, setTeachersError] = useState('');

  // Center rating state.
  const [ratingAvg, setRatingAvg] = useState(initialRating?.average ?? 0);
  const [ratingCount, setRatingCount] = useState(initialRating?.count ?? 0);
  ...
  // Server-rendered page: the initial center/teachers/rating are already fetched
  // on the server, so the bootstrap effect below only runs for later mounts.
  const ssrLoadedRef = useRef(true);
  ...
  const toggleFollow = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        dataCache.invalidatePrefix('centers:');
